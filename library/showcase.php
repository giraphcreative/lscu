		...
		foreach ( $slides as $key => $slide ) {
			if ( !empty( $slide["image"] ) ) {
				...
				if ( p_is_image( $slide["image"] ) ) {
					...
					$image = '<img src="' . $slide["image"] . '" alt="' . ( !empty( $slide['alt'] ) ? $slide['alt'] : ( !empty( $slide['title'] ) ? $slide['title'] : $slide['image'] ) ) . '">';
				} else {
			...
			<div class="slide<?php print ( $key == 0 ? ' visible' : '' ); ?>">
				<?php if ( !empty( $link ) ) { ?><a href="<?php print $link ?>" class="<?php print ( ( stristr( $link, 'vimeo' ) || stristr( $link, 'youtube' ) || stristr( $link, 'youtu.be' ) || stristr( $link, 'google.com/maps' ) ) && !stristr( $link, 'channel' ) ? 'lightbox-iframe' : '' ) ?>"><?php } ?>
				<?php print $image; ?>
